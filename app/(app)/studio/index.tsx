 ...
 * @version 2026.4.0 (Peak Performance)
 * @engine Skia + Reanimated + GestureHandler + WASM
 */
...
export default function StudioMaster() {
  ...
  useEffect(() => {
    if (Platform.OS === 'web') {
      const loadSkia = async () => {
        try {
          // WE REMOVED THE 'locateFile' CDN URL ENTIRELY.
          // By passing no arguments, Metro Bundler will automatically find
          // the canvaskit.wasm file hidden deep inside your local node_modules.
          await LoadSkiaWeb();
          setSkiaReady(true);
        } catch (err) {
          console.error(
            'WASM_BOOT_FAILURE: Could not load local CanvasKit.',
            err,
          );
        }
      };
  ...
  // Autosave Debounce (5 seconds of inactivity)
  useEffect(() => {
  ...
  if (!skiaReady) {
    return (
      <View style={styles.loaderContainer}>
        ...
        />
        <Text style={styles.bootText}>INITIALIZING GPU PIPELINE...</Text>
      </View>
    );
  }
